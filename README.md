...
var_dump($foo);  // foo
```
以上是 Container 的一个基本使用场景，当我们从 Container 获取内容（`entity`）时，默认情况下，容器会将传入的内容原封不动的返回。

像大部分容器组件一样，Container 也支持 `entity` 的 `definition`, definition 是一个内容的定义，它是一个返回 entity 的可调用对象，其中定义了 entity 的创建步骤。当 get 方法被调用时，Container 会先通过 definition 对象生成 entity，然后再将 entity 放入容器并返回。
```php
use ConstanzeStandard\Container\Container;
use ConstanzeStandard\Container\Entity;
...
```
Entity 构造方法的第三个参数设为 `true` 表示该 entity 是一个 definition（默认为 false）。

你也可以使用 Container 的 add 方法创建和添加 entity，它的参数与 Entity 的构造方法相同，并会返回创建的 entity 对象。以上示例的也可以用 add 方法完成。
```php
...
### 对 Entity 使用别名
我们可以为一个 entity 定义多个不同的别名，Container 将保存这些别名，并将它们指向同一个 entity.
```php
$container->alias('aliasName', 'entityName');
```
从 Container 中以 `aliasName` 获取的值 与 `entityName` 的内容是相同的。

### 判断和删除操作
Container 支持 PSR-11 中所定义的 has 方法，用来判断一个 entity 是否存在。同时 Container 也支持删除指定名称的 entity 的 `remove` 方法。
```php
...
### 实例化工厂 Make Factory
Container 的 `make` 方法允许你控制 entity 的获取方式，在通常情况下，我们用 get 方法获取的内容是全局唯一的，也就是说，entity 只会被创建一次，当我们再次获取时，Container 会直接将之前保存的值返回。如果我们希望重新生成 entity，就可以使用 make 方法。
```php
...
### 静态绑定的参数
有时，一些 definition 需要额外的数据源用来生成 entity，这时，我们可以通过 `addArguments` 方法为 definition  提前绑定一些`静态参数`。
```php
...
我们也可以通过 Container 的 `withEntityArguments` 方法，为之后的每一个 Entity 添加一个或多个统一的静态参数。
```php
...
### Entity 的提供者（Entity Provider）
在某些业务层面中，服务是以组的形式存在的，组与组之间有着比较独立的依赖关系，面对这种情况，将所有的 entries 和 definitions “一股脑”的装入容器是不太聪明的做法。而 Entity Provider 就是为这种场景而设计的功能。

一个 EntityProvider 承载了一系列针对 Container 的读写或配置操作：
```php
...
```
定义一个 EntityProvider，你只需要继承 `\ConstanzeStandard\Container\AbstractEntityProvider` 然后在 `register` 中操作 Container. 最后，将 EntityProvider 对象通过 Container 的 `addEntityProvider` 方法传入即可。

需要注意的是，你必须在 `provides` 属性中指明当前的 provider 提供了哪些 entity，否则将不会生效。

### 预加载的 provider (Bootable Entity Provider)
如果你希望在 provider 放入容器时，立刻注册一些 entity，或者做一些预先的配置工作，那么你可以选择使用 `BootableEntityProvider`，这是一种特殊的 EntityProvider，它有一个额外的方法 `boot`, 在 provider 加入容器时会立刻被执行。
```php
...
...
$container->addEntityProvider($entityProvider);
```
当 `addEntityProvider` 方法被调用时，`publicService` 就被立刻注册到容器中了，而 `foo` 则需要等到自身被调用时，才会进行注册。
